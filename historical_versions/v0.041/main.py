...
def test_detection(find_img=None, base_img=None):
    ...
    if not find_img:
        ...
        find_img = cv.imread("test_imgs/login_fb.png", cv.IMREAD_UNCHANGED) # 0, -1, cv.IMREAD_UNCHANGED, cv.IMREAD_REDUCED_COLOR_2
    ...
    result = cv.matchTemplate(base_img, find_img, cv.TM_CCORR_NORMED) # TM_CCOEFF_NORMED, TM_CCOEFF, TM_CCORR, TM_CCORR_NORMED, TM_SQDIFF, TM_SQDIFF_NORMED - BEST SO FAR : TM_CCORR_NORMED
    print(f"{result = }")
