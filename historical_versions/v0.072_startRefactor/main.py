...
# -- super duper temp for viewing the comp vision output for debugging --
temp_img = False

# so for this first multi-threading test
# what i actually want is
# for it to check if we are logged in
# and return true if we are else return false
# but pause for 5 - 10 seconds a time
# and we will run this just after login bot
...
def checker_bot_test(screenshot):
    ...
    global is_bot_active, is_logged_in, temp_img
    points, img = get_click_positions("test_imgs/play_btn_test_1_img.png", screenshot, 0.98, "rectangles")
    if points:
        print(f"Found Play Button - Login Success Confirmed")
        is_logged_in = True
        temp_img = img
    # allow us to start a new thread by using resetting global
    is_bot_active = False
