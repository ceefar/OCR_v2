...
while True:
    ...
    if not is_logged_in:
        ...
        # HUGE N0TE
        # - if you were already logged in its much easier (and different)
        # - so lets check for that first obvs

        completed = login_bot(screenshot)
        if completed:
            is_logged_in = True
...
# OK SO HONEST TO GOD AM PROBABLY GUNA DO A SMALL REFACTOR FOR THIS SO I CAN JUST START FROM A TECHNICALLY LOGGED IN STATE
# AND THEN JUST ADD IN THE LOGIN STUFF AFTER

# SO TBF JUST DO A QUICK LOGIN WHERE IT JUST DETECTS THE BUTTON AND LOGS IN
